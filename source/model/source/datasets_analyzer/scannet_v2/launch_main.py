...
def generate_keyframe(opt, dataset_dir):
    json_data = dict()
    
    print("generating keyframe metadata...")

    keyframe_dir = os.path.join(dataset_dir, "..").replace("\\", "/")
    if os.path.exists(os.path.join(keyframe_dir, "keyframes.json")):
        with open(os.path.join(keyframe_dir, "keyframes.json"), 'r') as f:
            json_data = json.load(f)

    length = len(os.listdir(os.path.join(dataset_dir, opt.filename, "color")))
    json_data[opt.filename] = [frame_count for frame_count in range(0, length)]
...
parser.add_argument('--task', default='test', help='task to perform')
...
def main():
    ...
    if config.images_rate != None:
        print(f"images rate is set to {config.images_rate}")
    if config.images_stride != None:
        print(f"images stride is set to {config.images_stride}")

    # load the data
    data_dir = os.path.join(
        config.dataset_dir, 
        '..', 
        'scans_' + ('train' if opt.task == 'train' else 'val' if opt.task == 'val' else 'test'), 
        opt.filename, 
        opt.filename + ".sens"
    ).replace("\\", "/")
    ...
    if opt.export_depth_images:
        sd.export_depth_images(os.path.join(output_path, 'depth'), frame_skip=images_stride)
    if opt.export_color_images:
        sd.export_color_images(os.path.join(output_path, 'color'), frame_skip=images_stride)
    if opt.export_poses:
        sd.export_poses(os.path.join(output_path, 'pose'), frame_skip=images_stride)
