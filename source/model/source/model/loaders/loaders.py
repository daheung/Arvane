...
import source.reconstruction.tsdf_fusion as tsdf_fusion

from PIL import Image

TARGET_RGB_IMG_SIZE = (480, 640)  # (height, width)
...
def load_scan(scan, keyframes_file=None):
    ...
    rgb_imgfiles = sorted(
        glob.glob(os.path.join(scan["image_dir"], "*.jpg")),
        key=lambda files: int(os.path.splitext(os.path.basename(files))[0])
    )
    
    gt_depth_imgfiles = sorted(
    ...
    for idx, is_ok in enumerate(good_pose):
        if not is_ok:
            print(f"{scan['scan_name']} - index [{idx:0>4d}] was skipped due to invalid pose.")
...
class UniversalToTensor:
    def __call__(self, x: torch.Tensor):
        if isinstance(x, (Image.Image, np.ndarray)):
            return torchvision.transforms.ToTensor()(x)

        if isinstance(x, torch.Tensor):
            x_cpy = torch.clone(x).to(dtype=torch.float32)
            if x_cpy.dim() == 3:
                x_cpy = x_cpy.permute(2, 0, 1)
            elif x_cpy.dim() == 2:
                x_cpy = x_cpy.unsqueeze(0)

            if torch.max(x_cpy) > 1.0:
                x_cpy /= 255.0

            return x_cpy

        raise TypeError(f"UniversalToTensor does not support {type(x)} type")